...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name)||empty($email)) {
        ...
        $sql = "SELECT * FROM yonghu WHERE name = :name OR email = :email";
        $stmt = $pdo->prepare($sql);
        $result = $stmt->execute(["name" => $name, "email" => $email]);
        if ($stmt->rowCount() > 0) {
            $x = "用户已被注册";
        }else{
            $sql = "INSERT INTO yonghu (name, password, email) VALUES (:name, :password, :email)";
            $hash_password = password_hash($password, PASSWORD_DEFAULT);
            $stmt = $pdo->prepare($sql);
            $result = $stmt->execute([":name" => $name, ":password" => $hash_password, ":email" => $email]);
            if ($result) {
                $x = "注册成功";
            }else{
                $x = '注册失败';
...
<div class="container">
    ...
    <form action="" method="post">
        ...
        <label>密码：</label>
        <input type="password" name="password" placeholder="小写字母或数字"><br><br>

        <label>确认密码：</label>
        <input type="password" name="password2" placeholder="请确认密码"><br><br>
